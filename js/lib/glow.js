...
// band: strength of the bright equator. Worlds whose camera pitches and
// banks (the slide) pass 0 — a level horizon line through a diving, rolling
// frame reads as a glitch, not a sky.
export function skyDome(radius = 320, band = 1) {
  const geo = new THREE.SphereGeometry(radius, 24, 16);
  const n = geo.attributes.position.count;
  const cols = new Float32Array(n * 3);
  for (let i = 0; i < n; i++) {
    const y = geo.attributes.position.getY(i) / radius; // -1..1
    const horizon = Math.max(0, 1 - Math.abs(y) * 1.7) * band;
    const v = 0.03 + horizon * horizon * 0.30 + (1 - band) * 0.05 * (1 - Math.abs(y));
