...
import { createTunnel } from './tunnel.js?v=428';
import { createSurfer } from './surfer.js?v=428';
import { createOrbit } from './orbit.js?v=428';
import { createBloom } from './bloom.js?v=428';
import { createTrail } from './trail.js?v=428';
import { createSignal } from './signal.js?v=428';
import { createRiver } from './river.js?v=428';
import { createFunhouse } from './funhouse.js?v=428';
import { createLavaLamp } from './lavalamp.js?v=428';
import { createPlasma } from './plasma.js?v=428';
import { createCherryLand } from './cherryland.js?v=428';
import { createSlinky } from './slinky.js?v=428';
import { createBlacktop } from './blacktop.js?v=428';
import { createWaterslide } from './waterslide.js?v=428';
import { createGarden } from './garden.js?v=428';
import { createPaint } from './paint.js?v=428';
import { createComets } from './comets.js?v=428';
...
export const WORLDS = {
  ...
  blacktop: { pulse: 0.85, rhythm: true, feetPerStep: 42, mode: 'DODGE', unit: 'GATES',
            rules: 'Steer with the mouse or arrows. Press and HOLD to speed up. Drive through the green gates — every one counts, and full speed counts double. Miss the striped walls — they cost you gates, and they’ve never once moved for anybody. Whoever runs the most gates when the song ends wins.', goal: 'run the midnight road — hold to open the throttle', label: 'BLACKTOP', create: createBlacktop },
  comets: { pulse: 0.9, rhythm: true, mode: 'DODGE', unit: 'STARS',
            rules: 'Steer with the mouse or arrows. Press and HOLD to fly faster. Fly through the silver stars — every one counts, and five in a row makes a constellation that counts for more. Stay away from the big red stars — they’re not the friendly kind. Whoever gathers the most stars when the song ends wins.',
  ...
  slide: { pulse: 0.8, rhythm: true, feetPerStep: 20, mode: 'DODGE', unit: 'RINGS',
            rules: 'Steer with the mouse or arrows. Press and HOLD to speed up. Lean through the green rings — every one counts, and full speed counts double. Steer AROUND the black holes — they swallow rings whole. Whoever threads the most rings when the song ends wins.', goal: 'ride the flume — hold to open the throttle', label: 'SLIDE', create: createWaterslide },
